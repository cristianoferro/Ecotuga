...
<?php
    ...
    if( count($_POST) > 0 ){
        ...
        mysqli_query($ligacao,"UPDATE norte SET lugar = '$lugar' , titulo = '$titulo' , descricao = '$descricao' , texto = '$texto'
        WHERE id = $id ")or die();
        ...
        if( $_FILES['imagem']['error'] == 0 ){
            ...
            mysqli_query($ligacao,"UPDATE norte SET imagem = '$imagem' WHERE id = $id");
    ...
    $consulta = mysqli_query($ligacao , "SELECT * FROM norte WHERE id = $id");
    //Guardar numa variável o registo encontrado pela consulta
    $norte = mysqli_fetch_assoc($consulta);
        ...
        <a href="norte.php" class="fechar_janela right">X</a>
          ...
          <div class="item-insert">
            ...
            <form  enctype="multipart/form-data" action="" method="post">
                ...
                <label>Lugar</label>
                <input type="text" name="lugar" value="<?php echo $norte['lugar']; ?>"/>

                <label>Título</label>
                <input type="text" name="titulo" value="<?php echo $norte['titulo']; ?>"/>

                <label>Descrição</label>
                <textarea name="descricao"><?php echo $norte['descricao']; ?></textarea>

                <label>Texto</label>
                <textarea name="texto"><?php echo $norte['texto']; ?></textarea>

                <label>Imagem</label>
                  <?php if($norte['imagem'] != ''){ ?>
                     <img src="../img/<?php echo $norte['imagem']; ?>" accept="image/*"/>
                  <?php } ?>
