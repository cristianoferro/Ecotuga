...
<?php
    ...
    if( count($_POST) > 0 ){
        ...
        mysqli_query($ligacao,"UPDATE sul SET lugar = '$lugar' , titulo = '$titulo' , descricao = '$descricao' , texto = '$texto'
        WHERE id = $id ")or die();
        ...
        if( $_FILES['imagem']['error'] == 0 ){
            ...
            mysqli_query($ligacao,"UPDATE sul SET imagem = '$imagem' WHERE id = $id");
    ...
    $consulta = mysqli_query($ligacao , "SELECT * FROM sul WHERE id = $id");
    //Guardar numa variável o registo encontrado pela consulta
    $sul = mysqli_fetch_assoc($consulta);
...
<body>
  ...
  <header id="top">
        ...
        <div class="janela_editar">
          <a href="sul.php" class="fechar_janela right">X</a>
          <div class="item-insert">

            <form  enctype="multipart/form-data" action="" method="post">

                <label>Lugar</label>
                <input type="text" name="lugar" value="<?php echo $sul['lugar']; ?>"/>

                <label>Título</label>
                <input type="text" name="titulo" value="<?php echo $sul['titulo']; ?>"/>

                <label>Descrição</label>
                <textarea name="descricao"><?php echo $sul['descricao']; ?></textarea>

                <label>Texto</label>
                <textarea name="texto"><?php echo $sul['texto']; ?></textarea>

                <label>Imagem</label>
                  <?php if($sul['imagem'] != ''){ ?>
                     <img src="../img/<?php echo $sul['imagem']; ?>" accept="image/*"/>
                  <?php } ?>
